...
addColumnButton.addEventListener("click", ()=>{
    if (table.querySelector("tr:first-child").querySelectorAll("td").length < MAX_LENGTH) for(let i = 0; i < table.rows.length; i++){
...
removeColumnButton.addEventListener("click", ()=>{
    if (table.querySelector("tr:first-child").querySelectorAll("td").length > MIN_LENGTH) for(let i = 0; i < table.rows.length; i++){
        const row = table.rows[i];
        row.deleteCell(-1);
        updateButton();
    }
